...
{
    if (indexPath.section == 0) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(onSelectPrinter:ip:)]) {
            Epos2DeviceInfo *info = (Epos2DeviceInfo *)[printerList_ objectAtIndex:indexPath.row];
            [self.delegate onSelectPrinter:info.getTarget ip:info.getIpAddress];
            self.delegate = nil;
...
- (void)connectDevice:(id)userInfo
{
    ...
    if (result == EPOS2_SUCCESS) {
        [self.delegate onSelectPrinter:(NSString *)BDAddress ip:@""];
        self.delegate = nil;
        [self.navigationController popToRootViewControllerAnimated:YES];
    }
    else {
